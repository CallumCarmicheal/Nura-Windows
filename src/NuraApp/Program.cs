...
static class Program {
    ...
    private static async Task RunAsync() {
        ...
        client.Monitoring.DeviceConnected += async (_, args) => {
            try {
                ...
                logger.SetHoistedSection("status", AnsiLine.From("Found new device, retrieving data for ", NuraGradient.Text(device.Info.DisplayName), "."));
        ...
        client.Monitoring.DeviceDisconnected += async (_, args) => {
            try {
                if (SelectedDevice == args.Device) {
                    SelectedDevice = null;
                    UpdateSelectedDeviceText();
                }

                logger.SetHoistedSection("status", $"Device disconnected : {args.Device.Info.DisplayName}.");

                await args.Device.StopMonitoringAsync().ConfigureAwait(false);
            } catch (Exception ex) {
                logger.WriteLine(
                    AnsiPart.Dim($"[{DateTime.Now:HH:mm:ss}] "),
                    AnsiPart.Error("[NuraLib] ERROR : "),
                    $"Device disconnect handler failed: {ex.Message}");
            } finally {
    ...
    private static void UpdateDevicesHoistText() {
        logger.SetHoistedSection("status", $"Discovered {Client?.Devices.Connected.Count ?? 0}, listening for changes.");
    }

    private static async Task FlashSelectedDeviceText(params AnsiPart[] parts) {
        logger.SetHoistedSection("current device", AnsiLine.From(parts));
        await Task.Delay(4000);

        // If we are still connected, show the text.
...
public static class NuraGradient {
    ...
    public static AnsiPart[] Text(string text, bool colourWhitespace = false) {
        ...
        if (text.Length == 1) {
            ...
            char.IsWhiteSpace(text[0]) && !colourWhitespace
                ? new AnsiPart(text)
                : new AnsiPart(text, new AnsiStyle(Foreground: Start))
        };
        }
        ...
        for (int i = 0; i < text.Length; i++) {
            ...
            Rgb colour = t <= 0.5
                ? Lerp(Start, Middle, t / 0.5)
                : Lerp(Middle, End, (t - 0.5) / 0.5);
